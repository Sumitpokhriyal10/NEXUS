...
form.addEventListener("submit", (event) => {
  ...
  const index1 = (name.length * surname.length * year) % 30;
  const fifth_message = recommendations[index1];

  const index2 = (day * month * year) % 20;
  const sixth_message = predictions[index2];

  result.innerText = `${first_message} ${second_message}
                    ${third_message} 
                    ${fourth_message} 
